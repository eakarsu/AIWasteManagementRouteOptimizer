...
function EnvironmentalImpact() {
  ...
  const handleAnalyze = async () => {
    ...
    setAiResult(null);
    try {
      const res = await fetch('/api/ai/environmental-impact', {
        method: 'POST',
        headers,
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', gap: 16, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          ...
          <button className="btn btn-ai" onClick={handleAnalyze} disabled={aiLoading}>
            <Sparkles size={16} /> Calculate Impact
          </button>
        </div>
      </div>

      <AIOutput data={aiResult} loading={aiLoading} title="Environmental Impact Results" />
